...
//Created by Josh, Conor, Aaron. Loosely based on tutorial found on Reddit.com
...
/*    //aaron declaring gamelevel, enemies, player health
    //private int gameLevel;
    private long enemyStartTime;
    private long enemyElapsed;
    private ArrayList<Enemy> enemies;
    private int enemiesNo = 0;
...
/*            //for loop looping through enemies
...
/*
            *//*
            ...
            //enemy added to system timer
            enemyElapsed = (System.nanoTime()-enemyStartTime)/1000000;

            //the enemies are added to the screen
            enemies.add(new Enemy(BitmapFactory.decodeResource(getResources(), R.drawable.enemies),
                    WIDTH + 10, HEIGHT/2, 45, 15, 10, 13));

            //for loop is used to check if an enemy collides with the player
            for(int i=0; i<enemies.size(); i++)
            {
                enemies.get(i).update();
                if(collision(enemies.get(i),player))
                {
                    //the enemy is removed from the screen
                    //the player's score is increased by 10 and their health decreases
                    enemies.remove(i);
                    player.setScore(player.score + 10);
                    player.setHealth((playerHealth -1));
                    break;
                }

                //once the enemy goes too far to the left of the screen it is removed
                ...
                for(int i = 0; i < enemiesNo; i++){
                    if(enemies[i].getVisibility()) {
                        ...
                        if(enemies(i).takeAim(playerShip.getX(), player.getLength())){

                            // If so try and spawn a bullet
                            if(enemyBullets[nextBullet].shoot(enemies(i).getX() + enimies(i).getLength() / 2, enemies(i).getY(), bullet.LEFT)) {
